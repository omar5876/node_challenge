...
describe('cuboid update', () => {
  ...
  it('should fail if not found the cuboid', async () => {
    const [newWidth, newHeight, newDepth] = [5, 5, 5];
    const response = await request(server).put('/cuboids/-1').send({
      width: newWidth,
      height: newHeight,
      depth: newDepth,
    });
    ...
    expect(response.status).toBe(HttpStatus.NOT_FOUND);
    expect(response.body).toBe('cuboid not found');
  });
...
describe('cuboid delete', () => {
  ...
  it('should not delete and return 404 status code when cuboids doesnt exists', async () => {
    const response = await request(server).delete('/cuboids/-1');
